<?php
	...
	function open_formatter($conn, $lections_location) { 
		$lection_id = fix_string($conn, trim($_SESSION['lection_id']));
		$row = get_first_Select_array($conn, "SELECT lection_link, is_file_opened FROM lections WHERE lection_id='$lection_id'", MYSQLI_NUM); 
		$filename = $row[0]; 
		$is_opened = $row[1];
